...
public class Estado implements Question<Boolean> {
    ...
    public Boolean answeredBy(Actor actor) {
        boolean columna = LST_FILA_DOCUMENTOS.of(actor.recall(CEDULA_ACTOR),actor.recall(FECHA_CREDITO)).resolveFor(actor).isPresent();
        if(!columna){
            columna =  LST_FILA_DOCUMENTOS.of(actor.recall(CEDULA_ACTOR),masUnMinuto(actor.recall(FECHA_CREDITO))).resolveFor(actor).isPresent();
        }
        return columna;
    }
}
